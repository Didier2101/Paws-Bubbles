        ...
        <section id="services" className="py-32 bg-zinc-950 relative overflow-hidden">
            <div className="container mx-auto px-6 relative z-10 mb-16 text-center">
                <motion.div
                    ...
                    viewport={{ once: true }}
                    className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-indigo-500/10 border border-indigo-500/20 text-indigo-400 text-[10px] font-black tracking-widest uppercase mb-4"
                >
                ...
                <h2 className="text-4xl md:text-6xl font-black mb-4 tracking-tighter uppercase italic">
                    ELIGE TU <span className="text-indigo-500 text-gradient">ESTILO</span>
                </h2>
            ...
            <div className="relative">
                <div className="flex overflow-hidden py-10">
                    ...
                    >
                        {isLoading ? (
                            [1, 2, 3, 4, 5].map(i => (
                                <div key={i} className="min-w-[400px] h-[300px] rounded-[48px] bg-white/5 animate-pulse" />
                            ))
                        ) : (
                            displayServices.map((service, index) => (
                                <div
                                    key={`${service.id}-${index}`}
                                    className="min-w-[400px] group relative bg-zinc-900 border border-white/5 rounded-[48px] p-10 hover:bg-indigo-600/10 hover:border-indigo-500/40 transition-all duration-500 shadow-2xl hover:shadow-indigo-500/10"
                                >
                                    {/* Icon & Index */}
                                    ...
                                    <div className="flex items-center gap-3 mb-6">
                                        <div className="text-2xl font-black text-indigo-500">${service.price.toLocaleString()}</div>
                                        <div className="px-3 py-1 rounded-full bg-white/5 text-[10px] font-bold text-gray-500 uppercase tracking-widest border border-white/5">{service.pet_size}</div>
                                    </div>
                                    ...
                                    <Link
                                        href="/reserva"
                                        className="inline-flex items-center gap-3 px-8 py-4 rounded-2xl bg-white/5 border border-white/10 text-[10px] font-black uppercase tracking-widest text-white hover:bg-white hover:text-black transition-all duration-300 group/btn"
                                    >
                ...
                >
                    <Link
                        href="/servicios"
                        className="group flex items-center gap-3 px-8 py-4 rounded-3xl bg-white/5 border border-white/10 text-xs font-black uppercase tracking-widest text-indigo-400 hover:bg-indigo-600 hover:text-white transition-all shadow-xl hover:shadow-indigo-600/20"
                    >
